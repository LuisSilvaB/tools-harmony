...
<template>
  <div class="w-full items-start flex-1 p-2 flex justify-center rounded-md gap-2 relative">
    <div v-if="props.dragState.isDragging"
      ...
      @drop="props.handleDrop" @dragstart="props.handleDragStart" @dragover="props.handleDragOver"
      @dragleave="props.handleDragLeave" @click="props.onOpenInputFile" @dragover.prevent>
      <p>Drop your images here...</p>
    </div>
    <div @dragstart="props.handleDragStart" @dragover="props.handleDragOver" @dragover.prevent
      class="flex flex-1 w-full h-fit overflow-y-auto max-h-[calc(100vh-150px)]">
            ...
            <td class="">
              <div class="flex flex-row gap-2 justify-center items-center">
                <Button size="icon" variant="default" class="flex flex-row gap-4 items-center p-2 text-white rounded"
                  @click="props.onOpenInputFile">
                  <TransformIcon class="text-white" />
                </Button>
                <Button size="icon" class="flex flex-row gap-4 items-center p-2 bg-blue-500 text-white rounded"
                  @click="props.onOpenInputFile">
                  <DownloadIcon class="text-white" />
                </Button>
                <Button size="icon" variant="destructive" class="flex flex-row gap-4 items-center p-2 rounded"
                  @click="props.onOpenInputFile">
